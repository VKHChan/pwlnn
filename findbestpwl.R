...
findbestpwl <- function(Inputdata, ANNOutput, ActualOutput, hiddenW, outputW, actfun="sigmoid", allpwlfull, error){
  ...
  # always use the lowest number of BP
  i <- 1
  repeat{
    minmseInd <- array(which.min(SAMatrix[i,]))
    minmseper <- SAMatrix[i,minmseInd]
    
    if(minmseper < error){
      thenode <- colnames(SAMatrix)[minmseInd]
      thepwl <- rownames(SAMatrix)[i]
    ...
    i <- i+1
    if(i>nrow(SAMatrix)) break()
  }
  
  # if no values in SA matrix is less than error
  ...
  if(mseper < minmseper){
    ...
    print(paste0("Best solution is found with a mse percentage of ", mseper, "%."))
    return(bestpwl)
  }
